...
SELECT id, idempotent_key, version, status, job_type, payload FROM outbox_items WHERE idempotent_key = $1 AND "status" = $2 LIMIT 1
`
...
SELECT id, idempotent_key, version, status, job_type, payload FROM outbox_items WHERE id = $1
`
